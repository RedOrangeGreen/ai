...
int main(int argc, char *argv[])
{
    ...
    operationsMenu->addAction(encodeAction);
    ...
    operationsMenu->addAction(decodeAction);
    QObject::connect(decodeAction, &QAction::triggered, [&]() {
        Operation::decode(encoded);
    });

    // Add a new action to change the key
    QAction *changeKeyAction = new QAction("Change &Key", &window);
    operationsMenu->addAction(changeKeyAction);
    QObject::connect(changeKeyAction, &QAction::triggered, [&]() {
        bool ok;
        QString newKey = QInputDialog::getText(&window, "Change Key",
                                               "Enter new key:", QLineEdit::Normal,
                                               Operation::getKey(), &ok);
        if (ok && !newKey.isEmpty()) {
            Operation::setKey(newKey);
            QMessageBox::information(&window, "Key Changed", "The key has been updated.");
        }
    });
