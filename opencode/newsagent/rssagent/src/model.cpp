                ...
                const ResponseCallback& callback)
{
    ...
    // Gemma 4 outputs tool calls in format: <|tool_call>call:func_name{args}<tool_call|>
    // Note: opening tag is <|tool_call>, closing tag is <tool_call|> (inconsistent pipe placement)
    // Parse this manually since bundled llama.cpp may not support it
    static const std::regex gemma4_tool_call(R"(<\|tool_call\>call:(\w+)\{([^}]*)\}<tool_call\|\>)");
    std::smatch match;
    if (std::regex_search(response, match, gemma4_tool_call)) {
        common_chat_msg parsed_msg;
        parsed_msg.role = "assistant";
        common_chat_tool_call tc;
        tc.id = "call_" + match[1].str();
        tc.name = match[1].str();
        // Parse args - empty dict if no args
        std::string args_str = match[2].str();
        tc.arguments = args_str.empty() ? "{}" : "{" + args_str + "}";
        parsed_msg.tool_calls.push_back(tc);
        return parsed_msg;
    }

    common_chat_parser_params parser_params;
    // Use explicitly configured format, or fall back to auto-detected format
    parser_params.format = config_.chat_format.value_or(params.format);

    auto parsed_msg = common_chat_parse(response, false, parser_params);
    parsed_msg.role = "assistant";
